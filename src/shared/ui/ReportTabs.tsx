...
function highlightSyntax(code: string): React.ReactNode[] {
  ...
  return lines.map((line, lineIdx) => {
    ...
    return (
      // biome-ignore lint/suspicious/noArrayIndexKey: 고정된 구문 코드 라인 순서
      <div key={`line-${lineIdx}`} className="flex">
        ...
        <span className="flex-1">
          {tokens.map((token, tokenIdx) => (
            // biome-ignore lint/suspicious/noArrayIndexKey: 고정된 구문 토큰 순서
            <span key={`token-${tokenIdx}`} className={token.className}>
              {token.text}
            </span>
          ))}
        </span>
  ...
  return (
    <div className="space-y-6 mt-10">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-geist-border">
        <h2 className="text-base font-bold text-black">{titleKo} — 실무 성능 종합 분석 정리</h2>
      </div>

      {/* 1. 개선 전 문제 상황 */}
      <GeistCard className="bg-white space-y-3 border-neutral-300 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="text-sm font-bold text-neutral-700 break-words">
            1. 개선 전 문제 상황 (테스트 시나리오)
          </div>
        </div>
        <pre className="p-4 rounded-lg bg-neutral-50 border border-neutral-200 text-black text-sm font-sans whitespace-pre-wrap break-words leading-relaxed font-medium">
          {report.before}
        </pre>
      </GeistCard>

      <div className="flex justify-center text-neutral-400 font-bold text-2xl">↓</div>
      ...
      <GeistCard className="bg-white space-y-3 border-blue-200 shadow-sm">
        ...
        <pre className="p-4 rounded-lg bg-blue-50/50 border border-blue-200 text-black text-sm font-sans whitespace-pre-wrap break-words leading-relaxed font-medium">
          {report.appliedTech}
        </pre>
      </GeistCard>
      ...
      {codeSnippet && (
        <GeistCard className="bg-white space-y-4 border-neutral-200 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="text-sm font-bold text-black break-words">4. 소스코드 구현 비교</div>
          </div>

          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 items-stretch max-w-full overflow-hidden">
            {/* 개선 전 코드 (IDE 다크 테마 구문 하이라이팅) */}
            <div className="flex flex-col min-w-0 max-w-full">
              <div className="flex items-center justify-between px-3.5 py-2 bg-[#21252b] font-mono text-sm border border-b-0 border-[#181a1f] rounded-t-lg">
                <span className="text-[#e06c75] font-bold truncate">개선 전 비최적화 소스</span>
                <span className="text-neutral-500 font-sans text-sm shrink-0 ml-2">BEFORE</span>
              </div>
              <pre className="flex-1 p-3 sm:p-4 bg-[#282c34] border border-[#181a1f] text-sm font-mono overflow-x-auto leading-relaxed shadow-inner rounded-b-lg">
            ...
            <div className="flex flex-col min-w-0 max-w-full">
              <div className="flex items-center justify-between px-3.5 py-2 bg-[#21252b] font-mono text-sm border border-b-0 border-[#181a1f] rounded-t-lg">
                <span className="text-[#98c379] font-bold truncate">최적화 완료 소스</span>
                <span className="text-neutral-500 font-sans text-sm shrink-0 ml-2">AFTER</span>
              </div>
              <pre className="flex-1 p-3 sm:p-4 bg-[#282c34] border border-[#181a1f] text-sm font-mono overflow-x-auto leading-relaxed shadow-inner rounded-b-lg">
      ...
      <GeistCard className="bg-white space-y-4 border-blue-200 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="text-sm font-bold text-blue-900 break-words">5. 측정 결과 및 회고</div>
        </div>
        ...
        <div className="space-y-4 font-sans text-sm">
          <div>
            ...
            <pre className="p-4 rounded-lg bg-blue-50/50 border border-blue-200 text-blue-900 font-mono text-sm whitespace-pre-wrap break-words leading-relaxed font-bold">
              {report.result}
            </pre>
          </div>
          ...
          <div className="pt-2">
            ...
            <div className="p-4 rounded-lg bg-neutral-50 border border-neutral-200 text-black text-sm whitespace-pre-wrap break-words leading-relaxed font-medium">
